...
class OpenProcessor:
    ...
    def llm(self,data):
        ...
        messages = [
            (
                ...
                #Instructions:
                - If there is amount such that 165 00 than it is 165.00, there is a decimal in between.
                -Return JSON only, no explanation, nothing.
                - BE EXTRA CAREFUL ABOUT DECIMALS, AND MENTION THEM PROPERLY
    ...
    def process_video(self,video_path):
        output_folder=r'C:\Users\Shashwat.Sharma\Documents\cyborgs\app\VideoFrames'
        results_path = r'C:\Users\Shashwat.Sharma\Documents\cyborgs\app\results2'
        ...
        if images:
            try:
                ...
                print("Error getting the video frame")
            try: 
                result_file = os.path.join(results_path, f"{video_name}_result.json")
                output = self.processor(selected_image)
                text = output
                output =self.extract_json(self.llm(text))
                print(output)
                try:
                    chat.add([text])
                except Exception as e:
    ...
    def process_image(self, image_path):
        results_path = r'C:\Users\Shashwat.Sharma\Documents\cyborgs\app\results2'
        try: 
            result_file = os.path.join(results_path, f"{image_path}_result.json")
            output = self.processor(image_path)
            text = output
            output =self.extract_json(self.llm(text))
            print(output)
    ...
    def process_videos(self,video_folder,resultants_folder):
        ...
        for video_file in video_files:
            ...
            video_name = os.path.splitext(video_file)[0]
            print(f"Processing video {video_name}")
            images = self.process_video(video_file)
            video_output_dir = os.path.join(output_folder, video_name)
            if images:
                try:
                    selected_image = os.path.join(video_output_dir, images[0])
...
if __name__ == '__main__':
    ...
    video_path=r"C:\Users\Shashwat.Sharma\Documents\cyborgs\Videos\video1.mp4"
    x = OpenProcessor().process_video(video_path)
    print(x)
